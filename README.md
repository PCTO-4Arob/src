...
To receive the input from the player we use a tracking function in openCV which we use like a mouse. The player have max 5 seconds to move on the operation that come in a random position in the screen, then the player have 4 possible solution to use and if he select the right one the game will prompt another expression.
